/* ════════════════════════════════════════════════
   discord.js — Détection Lucky Pull + Animation +
                Notification Discord via Bot
════════════════════════════════════════════════ */

/* ── Récupère le score de rareté le plus élevé déjà possédé
      (score = brawler.div × variante.chanceMult, plus c'est haut = plus rare)
      On exclut la clé actuelle (qui vient d'être incrémentée à 1 si c'était 0) ── */
function getMeilleurScoreRareteInventaire(excludeKey) {
  let best = 0;
  for (const [k, count] of Object.entries(etat.inventaire)) {
    if (k === excludeKey) continue;        // ignore le brawler qu'on vient de tirer
...
function estLuckyPull(brawler, variante, inventaireAvant) {
  ...
  // Cas 1 : brawler (+ variante) jamais obtenu auparavant
  const jamaisObtenu = !inventaireAvant[k] || inventaireAvant[k] === 0;
  if (jamaisObtenu) return true;

  // Cas 2 : score de rareté supérieur au meilleur score déjà en inventaire
  ...
  overlay.innerHTML = `
    <style>
      ...
      #lpCard::before {
        ...
        animation: lpRotate 3s linear infinite;
        border-radius: 50%;
      }
      ...
      .lpStar {
        ...
        border-radius: 50%;
        background: ${color};
        animation: lpStarFloat linear infinite;
        opacity: 0;
      }
...
async function envoyerNotifDiscord(brawler, variante, username) {
  const token     = window.DISCORD_BOT_TOKEN;
  const channelId = window.DISCORD_CHANNEL_ID;

  // Si les variables ne sont pas configurées, on skip silencieusement
  if (!token || !channelId || token.startsWith('${') || channelId.startsWith('${')) return;

  const v      = VARIANTES[variante];
  const color  = couleurVariante(brawler, variante);
  // Convertit #rrggbb en entier décimal pour Discord
...
function gererLuckyPull(brawler, variante, inventaireAvant) {
  ...
  // Récupère le pseudo depuis l'UI (leaderboard / cloud) si disponible
  const usernameEl = document.getElementById('leaderboardUsername')
    || document.getElementById('cloudUsername');
  const username = usernameEl ? usernameEl.value || usernameEl.textContent : null;

  envoyerNotifDiscord(brawler, variante, username);
}
